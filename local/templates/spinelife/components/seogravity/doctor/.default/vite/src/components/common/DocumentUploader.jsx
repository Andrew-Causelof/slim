...
export default function DocumentUploader({
  ...
  const handleFileUpload = async (e) => {
    ...
    const validatedFiles = files.filter((file) => {
      ...
      if (fileSizeMB > MAX_FILE_SIZE_MB) {
        const sizeMsg = `Файл слишком большой: ${
          file.name
        } (${fileSizeMB.toFixed(2)} МБ)`;
        setError(sizeMsg);
        notyf.error(sizeMsg);
        return false;
      }
    ...
    try {
      ...
      await Promise.all(
        validatedFiles.map((file) => uploadFile(userId, fieldName, file))
      );
  ...
  return (
    <div className="form_controls form_controls-files">
      <div
        className={`control control-files ${error ? 'control-files-err' : ''}`}
      >
        ...
        <div className="files">
          {userData.files &&
            userData.files[fieldName]?.map((file) => (
